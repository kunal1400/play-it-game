...
class Play_It_Game_Public {
		...
		if ( $post && $post->ID ) {
			...
			// if ( $isTeamGame && $isTeamGame == "true" && !empty($_GET['currentTeamId']) ) {
				wp_localize_script( $this->plugin_name.'play_it_js', 'current_env', array(
					...
					"current_team_id" => !empty($_GET['currentTeamId']) ? $_GET['currentTeamId'] : 0,
	...
	public function page_load_actions() {
		...
		if ( is_user_logged_in() && 'page' === get_post_type() AND is_singular() ) {
			/**
			* Check if current game is team game or not, If it is not team game then create a team 
			* in background.
			**/
			$isTeamGame = get_post_meta( $post->ID, 'is_team_game', true );
			if ( $isTeamGame && $isTeamGame == "true" ) {
				// Do Nothing if it is team game
			}
			else {
				...
				if ( !empty($currentUserId) ) {					
					...
					if ( !is_array($userTeams) ) {
						$this->manageTeam($currentUser->data->user_login, $currentUser->data->user_email, $post->ID, $currentUserId);
					}
				}
			}
	...
	public function game_home_page_cb( $attributes ) {
		...
		$gameHomePageId = $post->ID;
		$isSubLevel = false;
		if ( !empty($post->post_parent) ) {
			$gameHomePageId = $post->post_parent;
			$isSubLevel = true;
		}
		...
		**/
		$gameLevels = get_pages(array(
			'child_of' => $gameHomePageId
		));

		/**
		* #2: Getting the multipler
		**/
		$score_multipler = get_post_meta( $gameHomePageId, 'score_multipler', true);
		if (!$score_multipler) {
			$scoreMultipler = 10;
		} else {
		...
		if( !is_user_logged_in() ) {
			...
			**/
			$otherLevels = $this->getOtherLevels($gameHomePageId);

			/**
			* #6: Checking if current game is team game or not
			**/
			$isTeamGame = get_post_meta( $gameHomePageId, 'is_team_game', true );
			if ( $isTeamGame && $isTeamGame == "true" ) {
				$isTeamGame = true;
			}
			else {
			...
			$teams = $this->getAllTeams( $gameHomePageId );
			if ( is_array($teams) && count($teams) > 0 ) {
				$html .= "<h3>".$atts['table_label']."</h3>";
				$html .= "<table style='font-family:".$atts['font_family']."' id='teams'>";
				$html .= "<thead>";
				...
				$html .= "<th style='background-color:".$atts['heading_background_color']."' width='100'>".$atts['score_label']."</th>";
				$html .= "<th style='background-color:".$atts['heading_background_color']."' width='20'>".$atts['levels_label']."</th>";
				$html .= "<th style='background-color:".$atts['heading_background_color']."'>".$atts['members_label']."</th>";
				if (!$isSubLevel) {					
				...
				foreach ($teams as $i => $team) {
					...
					**/
					...
					// User is associated to this team then show Start Playing button
						...
						$nextLevelLink = $this->getGameNextLevelLink( $gameHomePageId );
						if ( !$nextLevelLink ) {
	...
	**/
	...
	/**
	* This function is useful if current game is private game
	**/
	public function getUserTeamsInGame( $gameId, $userId ) {
		global $wpdb;
		$tblname = $wpdb->prefix . 'gm_teams';
		$sql = "SELECT * FROM $tblname WHERE game_id=$gameId AND created_by=$userId";
		return $wpdb->get_row($sql, ARRAY_A);
	}
	...
	**/
	...
	public function redirect_button_cb( $atts ) {
		...
		return '<a href="'.$attributes['redirect_url'].'" type="button" class="btn btn-primary">'.$attributes['label'].'</a>';
	}
